...
var appCtl; // set as cfg var

function WebBot(configuration) {// the factory of a obj having this module as closure
    ...
    controller.openSocketServer = function(server) {
		...
		// ak47  TR21 only :
		//  instead of calling controller.ingest:  call appCtl that cb to  controller.ingest
             //appCtl.getpost(bot, message);//, ws);  // after cb to original call : 
    ...
    controller.middleware.categorize.use(function(bot, message, next) {
        ...
        // ak47 , TR21 , TR22 : comment following 
...
console.log('** bot:  send called with message: ',message);
		...
		/* moved to wssend()
			...
			//	TR22  leave original path :
			this.wssend(message,cb);// std with no app ctl  call before the app controller

			//	TR21
			//appCtl.cb(message,cb,bot);
			// appCtl.cb1(message,function(msg){bot.wssend(msg,cb)});// or that.wssend() after a that=this in this function (enclosure of the cb');
...
// ak47
 	bot.wssend= function (message,cb){
		console.log('wssend , appCtl asked bot to send to ws the message : ',message);
                    try {
                        if (bot.ws && bot.ws.readyState === WebSocket.OPEN) {
                            bot.ws.send(JSON.stringify(message), function(err) {
                                if (cb) {
				console.log('wssend, cb called after message was SEND ');
                                    return cb(err, message);
                                }
                            });
                        } else {
...
// ak47,TR21, TR22 : set following appctl reference

	controller.setAppCtl = function(ctl) {// init staff will set closure static cfg var 
		//console.log('** bot  getpost is : ',ctl.getpost);
		appCtl= ctl;
	};
