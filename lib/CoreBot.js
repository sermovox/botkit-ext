...
function Botkit(configuration) {
    ...
    botkit.receiveMessage = function(obot, omessage) {
        ...
        botkit.middleware.receive.run(obot, omessage, function(err, bot, message) {
            if (err) {
                ...
                bot.findConversation(message, function(convo) {// by user x channel
                    if (convo) { //botkit.debug('\n **M RECEIVED MESSAGE, found a convo after receive middleware :',message);
			//botkit.debug('\n **M found a convo ,script ',convo.context.script_name,',after receive middleware RECEIVED MESSAGE:',message);
                   	botkit.debug('\n **M engaged convo receive message ,script/command ',convo.context.script_name,',after receive middleware RECEIVED MESSAGE:',message);
                        convo.handle(message);
                    } else { //botkit.debug('\n **M RECEIVED MESSAGE, triggering ',message.type,' after receive middleware RECEIVED MESSAGE:',message);
			botkit.debug('\n **M after receive middleware ,if the message dont find a convo waiting , test matchers on event queue (triggering on message.type=eventqueue) : ',message.type,' after receive middleware RECEIVED MESSAGE:',message);
    ...
    function Conversation(task, message) {
        ...
        this.handler = null;// cb in ask and addquestion : addQuestion = function(message, cb, capture_options, thread).the answer will be managed in convo by this cb 
        this.responses = {};
	...
	// response=user_message_responding2_engaged_question , add response.question=the question in current engaged ask(question, cb, capture_options={key:'key'}) 
	// add entry in  responses['key'] = response
        ...
        this.handle = function(message) {
            ...
            botkit.debug('HANDLING MESSAGE IN a waiting CONVO', message);
            // do other stuff like call custom callbacks in ask and addQuestion(question,  cb=handler, capture_options)
            if (this.handler) {
                this.capture(message, function(message) {//capture the key (capture_options{key:'key'}) of handler then goon 
                    /*
        ...
        this.on = function(event, cb) {
            botkit.debug('**ON corebot.conversation.on, Setting up a handler for', event);
		...
		botkit.debug('**TRIGGER corebot.conversation.trigger fired for event', event,' result : ',res );
                    if (res === false) {
                        return;
                    }
                }
        ...
        this.tick = function() {
            ...
            if (this.isActive()) {
                if (this.processing) {
                        ...
                        if (typeof(this.messages[0].timestamp) == 'undefined' ||
                            ...
                            if (message.conditional) {
                                ...
                                if (message.text || message.attachments || message.attachment) {
                                    ...
                                    botkit.debug('** convo.tick() asks bot to send the message : bot.reply(): ',message);
				 ...
				 botkit.debug('** convo.tick() cb from send: so a msg was sent a message calling bot.reply: ', sent_message);
                                            that.sent[that.sent.length - 1].sent = true;
                                            ...
                                            that.trigger('sent', [sent_message]);// seems not used with Websocket
                        ...
                        if (this.isActive() && !this.messages.length && !this.handler && !this.processing) {
                            botkit.debug('** convo.tick() asks to end immediately the convo: ');
 
                            this.stop('completed');
                        }

                    } else if (this.sent.length) { // sent at least 1 message
                        botkit.debug('** convo.tick() asks to end  the convo: ');
                        this.stop('completed');
                    }
                }
            }
    ...
    function Task(bot, message, botkit) {
        ...
        this.conversationEnded = function(convo) {
            ...
            botkit.middleware.conversationEnd.run(this.bot, convo, function(err, bot, convo) {
                ...
                botkit.debug('** >  [End]  middleware.conversationEnd ended, [End] ', convo.id, ' Conversation with ',
                ...
                botkit.debug('** >  [End]  task still keeps ',actives,' active convos');
        ...
        this.on = function(event, cb) {
            botkit.debug('**ON corebot.task.on, Setting up a handler for', event);
            var events = event.split(/\,/g);
        ...
        this.trigger = function(event, data) {
            if (this.events[event]) {
                for (var e = 0; e < this.events[event].length; e++) {
                    var res = this.events[event][e].apply(this, data);
		 botkit.debug('**TRIGGER corebot.task.trigger fired for', event,' result : ',res );
                    if (res === false) {
                        return;
                    }
                }
    ...
    botkit.hears = function(keywords, events, middleware_or_cb, cb) {
        ...
        /*
         * the third parameter is EITHER a callback real hears handler
        ...
        for (var e = 0; e < events.length; e++) {// register the matcher for the hears handler cb in all hears event queues
            // ***** >> the matcher for the handler cb : check pattern on message , if match call heard,the handler and finally the heard_trigger event  
 


            // register (.on) the event handler on a closure where there some fields(or the param) are set ( like a new obj with its fields and the method) 
            (function(keywords, test_function,e) {
                // here the params keywords, test_function, e  
                // became  (like) static var of enclosure of  botkit.on cb
                // neednt copy the params to a var !!

		        console.log('**T corebot.hears() will add a item on ',events[e],' event to match  pattern ',keywords,' on  event ',events[e]);
                botkit.on(events[e], 

                    // ***** >> the matcher for the hears handler cb : check pattern on message , if match call heard,the handler and finally the heard_trigger event  
                    function(bot, message) { 
                   // *** cb exposed to the obj botkit , method on :
                    //  when obj botkit will do some task on some produced/find/calculated param obj related to method 'on' 
                    //  will ask this cb to process the param obj according to its method code referring the obj set in its closure

                    //  >> usually the cb will process 
                    //          using std config obj in cb scope at the time of cb call  
                    //              some related to this specific cb function (its enclosure set at the time of method 'on') 
                    //      according the cb method
                    //      some main obj passed in param (the session/status), 
                    //      related to other param obj
                    //      according to cfg obj in cb scope ( at the time of cb call)

            //  >> ON THIS inner CB we see the hosting var at the call time ,so var e ( like a cb param)  will be changed 

			botkit.debug('** testing hears event entry with tester(botkit.pattMatching or intentskills.intentsMatching ) : ',test_function,' on pattern/preferredInt ',keywords,'   I HEARD', message.text);
                    if (test_function && test_function(keywords, message)) {
                        botkit.debug('**** GOT!  matching hears on ',
                                    events[e],// e is copied as var in the enclorure so is not lost !!!
                                    ' event,  I HEARD msg : ',message,' matching : ', keywords,'\n so run heard mid on message then the hears_handler(bot,message) on this=botkit=controller then finally trigger heard_trigger ');
                        botkit.middleware.heard.run(bot, message, function(err, bot, message) {

                            // added error management copyng from: botkit.middleware.receive.run(obot, omessage, function(err, bot, message) {...
                            // seems that also if the error is got in this !err code the middleware recall this ending cb with error set !!!!!                          
                            if (err) {
                                // log the error to console, so people recognize it
                                console.error('An error occurred in the receive middleware: ', err);
                                /*
                                ...
                                botkit.trigger('pipeline_error', [err, bot, message, 'heard']);
                            } else {
                                console.log('***  heard middleware finished its chain, end cb called with err',err,message,keywords);
                                cb.apply(this, [bot, message]);
                                botkit.trigger('heard_trigger', [bot, keywords, message]);
                            }
                        });
                        return false;
                    }else return true;// continue to test next handler ( x another hears()) x the event events[e] 
                }, true,keywords);// true : is_hearing
            })(keywords, test_function,e);
        }
 ...
 //botkit.on = function(event, cb, is_hearing) {
        botkit.on = function(event, cb, is_hearing,pattern) {// the convo/form/intent/hears name
        // cb is the matcher registered on the events of the  list. it will check if message matches then goon the realhandler as registered on hears() 

        botkit.debug('**ON start corebot.botkit.on x event ', event);
      ...
      var events = (typeof(event) == 'string') ? event.split(/\,/g) : event;
        ...
        for (var e in events) {
            ...
            var matcher_={
                callback: cb,// the matcher
                type: is_hearing ? 'hearing' : 'event'
            };
            if(pattern)matcher_.pattern=pattern;
            var index=this.events[events[e]].length;
            this.events[events[e]].push(matcher_);
            botkit.debug('**ON corebot.botkit.on Setting up a handler for event queue ', event, 'index : ',index,' is_hearing : ',is_hearing);
        }
        return this;
    };

    botkit.trigger = function(event, data) {// data > bot, message
        // test first hears then other, the e-esimo event matchers hearing[e].callback  ( the handler)
        ...
        botkit.debug('**TRIGGER botkit.trigger firing hears handler with pattern : ',hearing[e].pattern,
        ' event queue : ', event,' index : ',e,' on data(bot,message): ',data );
                var res = hearing[e].callback.apply(this, data);
		//botkit.debug('**TRIGGER botkit.trigger fired for hears on event ', event,' on (bot,message) : ',data, ' with result : ',res);
        botkit.debug('**TRIGGER botkit.trigger fired for hears event matcher pattern : ',hearing[e].pattern,
        ' event queue : ', event,' index : ',e,' on data(bot,message): ',data, ' match : ',!res );
        if (res === false) {// at first match exit
                    return;
                }
            }
            ...
            if (handlers.length) {
                ...
                botkit.middleware.triggered.run(data[0], data[1], function(err, bot, message) {
                    for (var e = 0; e < handlers.length; e++) {
                        ...
                        botkit.debug('**TRIGGER botkit.trigger fired for non hears event matcher ' ,// command : ', handlers[e].cmd,
                        ' event: ', event, ' match : ',!res,' index : ',e,' on data(bot,message): ',data );
                        if (res === false) {
                            return;
                        }
                    }
	...
	botkit.log(' **M middleware spawn run on this new controller: ' );
